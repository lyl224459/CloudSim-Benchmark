...
class ExperimentOutputContextConfigTest {
    ...
    @Test
    fun `csv disabled suppresses trial file output`() = runBlocking {
        val tempDir = Files.createTempDirectory("output-context-disabled").toFile()
        try {
            val context = ExperimentOutputContext.from(
    ...
    @Test
    fun `csv delimiter is applied when enabled`() = runBlocking {
    ...
    fun `creates experiment directory and saves resolved config`() {
        ...
        try {
            val systemConfig = SystemConfig.createDefault().copy(
