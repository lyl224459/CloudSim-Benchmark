...
class ExperimentOutputContextTest {
    @Test
    fun `concurrent trial writes keep a single header and complete rows`() = runBlocking {
    ...
    @Test
    fun `child output context inherits csv settings and writes into child directory`() = runBlocking {
        val tempDir = Files.createTempDirectory("experiment-output-child").toFile()
        try {
            val parent = ExperimentOutputContext(
