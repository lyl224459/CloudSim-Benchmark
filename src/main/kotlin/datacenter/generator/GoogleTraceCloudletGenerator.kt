...
class GoogleTraceCloudletGenerator(
    ...
    private fun loadTraceData() {
        try {
            ...
            Logger.info("加载Google Trace数据: $traceFilePath")
            ...
            file.forEachLine { line ->
                ...
                try {
                    ...
                    if (record != null &&
                        record.timestamp >= timeWindowStart &&
                        record.timestamp <= timeWindowEnd) {
            ...
            Logger.info("成功加载 ${traceData.size} 条Google Trace记录")
            isLoaded = true

        } catch (e: Exception) {
            Logger.error("加载Google Trace数据失败: ${e.message}", e)
