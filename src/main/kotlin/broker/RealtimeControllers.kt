...
class RealtimeAdmissionController(
    ...
    fun decide(
        ...
        if (nodeStates.isNotEmpty() && nodeStates.none { it.acceptingWork }) {
            val reason =
                if (nodeStates.any { !it.resourceAcceptingWork }) {
...
class RealtimeTenantController(
    ...
    fun retrySuccessByTenant(
        ...
        val rates =
            retryRecords.groupBy { it.tenantId.value }.values.map { tenantRecords ->
