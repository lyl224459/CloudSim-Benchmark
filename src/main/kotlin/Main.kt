...
internal fun resolveRun(command: CliParser.RunCommand): ResolvedExperimentConfig =
    resolveRunCommand(command)

internal fun parseBatchAlgorithms(algorithmNames: List<String>): List<BatchAlgorithmType> =
    registryParseBatchAlgorithms(algorithmNames)
...
): config.ConfigurationManager.LoadedConfigs =
    cli.applyRunOverrides(configs, command, selectionAlgorithmConfigs)