...
data class RealtimeAlgorithmResult(
    ...
    val fairnessViolationCount: Int,
    val tenantSlaPenalty: Double,
    val dominantResourceFairnessIndex: Double,
    val costSlaTradeoffScore: Double,
    val retrySuccessByTenant: Double,
...
data class RealtimeAlgorithmStatistics(
    ...
    val tenantBudgetRejectedCount: StatisticalValue,
    val tenantFairnessIndex: StatisticalValue,
    val fairnessViolationCount: StatisticalValue,
    val tenantSlaPenalty: StatisticalValue,
    val dominantResourceFairnessIndex: StatisticalValue,
    val costSlaTradeoffScore: StatisticalValue,
    val retrySuccessByTenant: StatisticalValue,
...
class RealtimeComparisonRunner(
    ...
    private fun runRealtimeAlgorithm(
        ...
        return RealtimeAlgorithmResult(
            ...
            tenantBudgetRejectedCount = metrics.tenantBudgetRejectedCount,
            tenantFairnessIndex = metrics.tenantFairnessIndex,
            fairnessViolationCount = metrics.fairnessViolationCount,
            tenantSlaPenalty = metrics.tenantSlaPenalty,
            dominantResourceFairnessIndex = metrics.dominantResourceFairnessIndex,
            costSlaTradeoffScore = metrics.costSlaTradeoffScore,
            retrySuccessByTenant = metrics.retrySuccessByTenant,
    ...
    private fun calculateRealtimeMetrics(
        ...
        val totalSlaPenalty = slaPenalty + broker.getPreemptionPenalty() + tenantSlaPenalty
        ...
        return RealtimeMetrics(
            ...
            tenantBudgetRejectedCount = broker.getTenantBudgetRejectedCount(),
            tenantFairnessIndex = broker.getTenantFairnessIndex(cloudletList),
            fairnessViolationCount = broker.getFairnessViolationCount(),
            tenantSlaPenalty = tenantSlaPenalty,
            dominantResourceFairnessIndex = broker.getDominantResourceFairnessIndex(),
            costSlaTradeoffScore = broker.getCostSlaTradeoffScore(cost, totalSlaPenalty),
            retrySuccessByTenant = broker.getRetrySuccessByTenant(cloudletList),
        ...
        val executionTime = measureTimeMillis {
            ...
            val summaryData = results.map { r ->
                mapOf(
                    ...
                    "TenantBudgetRejectedCount" to r.tenantBudgetRejectedCount,
                    "TenantFairnessIndex" to r.tenantFairnessIndex,
                    "FairnessViolationCount" to r.fairnessViolationCount,
                    "TenantSlaPenalty" to r.tenantSlaPenalty,
                    "DominantResourceFairnessIndex" to r.dominantResourceFairnessIndex,
                    "CostSlaTradeoffScore" to r.costSlaTradeoffScore,
                    "RetrySuccessByTenant" to r.retrySuccessByTenant,
            ...
            outputContext.saveSummaryResults(
                ...
                listOf(
                    ...
                    "TenantQuotaRejectedCount", "TenantBudgetRejectedCount", "TenantFairnessIndex",
                    "FairnessViolationCount", "TenantSlaPenalty", "DominantResourceFairnessIndex",
                    "CostSlaTradeoffScore", "RetrySuccessByTenant", "CrossRackAssignmentCount",
    ...
    private fun failedRealtimeSummary(algorithmName: String): RealtimeRunSummary {
        val failedResult = RealtimeAlgorithmResult(
            ...
            tenantBudgetRejectedCount = Int.MAX_VALUE,
            tenantFairnessIndex = Double.NaN,
            fairnessViolationCount = Int.MAX_VALUE,
            tenantSlaPenalty = Double.NaN,
            dominantResourceFairnessIndex = Double.NaN,
            costSlaTradeoffScore = Double.NaN,
            retrySuccessByTenant = Double.NaN,
    ...
    private fun averageRealtimeResults(runResults: List<RealtimeAlgorithmResult>): RealtimeAlgorithmResult {
        return RealtimeAlgorithmResult(
            ...
            tenantBudgetRejectedCount = runResults.map { it.tenantBudgetRejectedCount }.average().roundToInt(),
            tenantFairnessIndex = runResults.map { it.tenantFairnessIndex }.average(),
            fairnessViolationCount = runResults.map { it.fairnessViolationCount }.average().roundToInt(),
            tenantSlaPenalty = runResults.map { it.tenantSlaPenalty }.average(),
            dominantResourceFairnessIndex = runResults.map { it.dominantResourceFairnessIndex }.average(),
            costSlaTradeoffScore = runResults.map { it.costSlaTradeoffScore }.average(),
            retrySuccessByTenant = runResults.map { it.retrySuccessByTenant }.average(),
    ...
    private suspend fun executeRealtimeSingleRunAsync(
        ...
        outputContext.saveAlgorithmTrialResult(
            ...
            mapOf(
                ...
                "TenantBudgetRejectedCount" to result.tenantBudgetRejectedCount.toDouble(),
                "TenantFairnessIndex" to result.tenantFairnessIndex,
                "FairnessViolationCount" to result.fairnessViolationCount.toDouble(),
                "TenantSlaPenalty" to result.tenantSlaPenalty,
                "DominantResourceFairnessIndex" to result.dominantResourceFairnessIndex,
                "CostSlaTradeoffScore" to result.costSlaTradeoffScore,
                "RetrySuccessByTenant" to result.retrySuccessByTenant,
    ...
    private fun calculateRealtimeStatistics(
        ...
        return RealtimeAlgorithmStatistics(
            ...
            tenantBudgetRejectedCount = stats(results.map { it.tenantBudgetRejectedCount.toDouble() }),
            tenantFairnessIndex = stats(results.map { it.tenantFairnessIndex }),
            fairnessViolationCount = stats(results.map { it.fairnessViolationCount.toDouble() }),
            tenantSlaPenalty = stats(results.map { it.tenantSlaPenalty }),
            dominantResourceFairnessIndex = stats(results.map { it.dominantResourceFairnessIndex }),
            costSlaTradeoffScore = stats(results.map { it.costSlaTradeoffScore }),
            retrySuccessByTenant = stats(results.map { it.retrySuccessByTenant }),
                ...
                val row = if (runs > 1) {
                    if (stat != null) {
                        ...
                        listOf(
                            ...
                            result.tenantBudgetRejectedCount, Double.NaN,
                            result.tenantFairnessIndex, Double.NaN,
                            result.fairnessViolationCount, Double.NaN,
                            result.tenantSlaPenalty, Double.NaN,
                            result.dominantResourceFairnessIndex, Double.NaN,
                            result.costSlaTradeoffScore, Double.NaN,
                            result.retrySuccessByTenant, Double.NaN,
                    ...
                    listOf(
                        ...
                        result.tenantBudgetRejectedCount,
                        result.tenantFairnessIndex,
                        result.fairnessViolationCount,
                        result.tenantSlaPenalty,
                        result.dominantResourceFairnessIndex,
                        result.costSlaTradeoffScore,
                        result.retrySuccessByTenant,
