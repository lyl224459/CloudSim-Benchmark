...
object CommandExecutor {
    private val coordinator = CommandExecutionCoordinator(CommandExecutionServices.production())
    ...
    suspend fun execute(command: CliParser.Command) {
        when (command) {
            ...
            is CliParser.RunCommand -> coordinator.run(command)
        }
    }

    internal fun configureLogging(systemConfig: config.SystemConfig) {
    ...
    internal fun validateEnvironment() {
        try {
