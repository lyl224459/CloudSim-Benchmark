...
| `PSO_REALTIME` | | Metaheuristic | `population`, `maxIter` | 达到阈值后对 accepted candidates 优化。 |
| `WOA_REALTIME` | | Metaheuristic | `population`, `maxIter` | 达到阈值后对 accepted candidates 优化。 |
...
`--algorithms ALL` 展开当前 mode 下所有启用算法。算法启用状态和默认参数来自：
...
- realtime 优化型对比：`MIN_LOAD,PSO_REALTIME,WOA_REALTIME`。
- 大任务数性能趋势：先用 `RANDOM` 或 `MIN_LOAD` 验证链路，再加入 metaheuristic。
