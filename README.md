...
CloudSim Plus 通过 `third_party/cloudsimplus` submodule 源码构建。普通本地、PR、release 和 container 构建默认严格使用 `gradle/cloudsimplus.lock` 中的固定 ref、commit 与版本；默认锁定模式只增量验证 checkout commit、父仓库 gitlink 和 POM version，不依赖本地 tag 对象，也不执行网络或 checkout，缺失 submodule 时先运行 `git submodule update --init --recursive`。Maven 依赖默认缓存到 `~/.m2/repository`，构建后仅将锁定版本的 JAR/POM 写入 `build/cloudsimplus-raw-m2`，清理 manifest 后的可解析 JAR/POM 写入 `build/cloudsimplus-m2`。可用 `-Pcloudsimplus.mavenCacheDir=<dir>` 覆盖依赖缓存位置。兼容性测试可用 `-Pcloudsimplus.ref=<tag-or-sha>` 指定 ref，或用 `-Pcloudsimplus.autoUpdate=true` 测试最新 semver release；这两种动态模式才会执行 fetch/checkout，确认兼容后通过 `updateCloudSimPlusLock` 更新 lock 并提交对应 submodule gitlink。`-Pcloudsimplus.offline=true` 使用当前 checkout，受限网络可用 `-Dorg.gradle.project.cloudsimplus.gitProxy=http://host:port` 指定代理。PowerShell 下 dotted Gradle 参数需要加引号，例如 `'-Dorg.gradle.project.cloudsimplus.gitProxy=http://host:port'`。源码构建任务支持 Gradle configuration cache；如遇网络失败，优先指定代理而不是关闭 configuration cache。Maven 源码构建会自动追加 JDK 25 兼容 JVM 参数，并保留用户已有 `MAVEN_OPTS`。
...
镜像默认以固定 UID/GID `10001` 的非 root 用户运行，根文件系统可设为只读，唯一需要持久写入的目录是 `/app/runs`。
`run.cmd podman` 和 `./run podman` 会将宿主机的 `runs/` 挂载到该目录。
...
发布流程由 `.github/workflows/release.yml` 在 `v*.*.*` 标签或手动触发时执行，复用 Gradle release package tasks 生成可执行 JAR、Windows zip、Unix tar.gz、源码包、SBOM、许可证报告和清单，并使用最小运行时上下文推送容器镜像到 GHCR。GitHub 原生 attestation 会为 release assets、fatJar/SBOM 和 GHCR 镜像 digest 生成可验证的 provenance。发布前验收清单见 `docs/release-readiness.md`。
...
依赖供应链由 Gradle dependency verification、Dependabot 和 OSV workflow 共同检查。`generateSupplyChainReports` 基于发布 runtime classpath 生成 CycloneDX SBOM 与第三方许可证报告，`checkLicense` 按 `gradle/allowed-licenses.json` 的精确允许列表阻断未知或新增许可证；许可证插件当前不支持 configuration cache，因此该任务作为独立发布步骤运行。
...
项目自身代码使用 MIT License。运行时依赖 CloudSim Plus 使用 GPLv3，分发组合产物时必须同时遵守其许可证要求；完整第三方许可证清单由 `generateSupplyChainReports` 生成。
